...
final class RecentActivityCard: CardView {
  ...
  init(winStreak: Binding<UInt>, matchesPlayed: Binding<Int>, matchesWon: Binding<Int>) {
    ...
    matchesThisWeekText = matchesPlayed.map { String($0) }
    _ = matchesThisWeekText.map { Int($0) == 1 ? " match" : " matches" + " this week" }
    
    winsThisWeekText = matchesWon.map { String($0) }
      _ = winsThisWeekText.map { Int($0) == 1 ? " win" : " wins" + " this week" }
    super.init()
  }
